<h1>Hi, I’m Daniel Lehmann 👋</h1>
  <p>
    <img src="https://img.shields.io/github/followers/lehmand?label=Followers&style=social" alt="Followers">
    <img src="https://img.shields.io/github/stars/lehmand?label=Stars&style=social" alt="Stars">
  </p>
  <hr>

  <h2>About Me</h2>
  <p>Hello! I’m Daniel Lehmann, a 37-year-old software developer based in Mannheim, Germany. Building efficient and scalable web applications and passionate about creating clean, maintainable code and delivering high-quality solutions.</p>
  <ul>
    <li>🌱 I’m continuously learning new technologies and best practices.</li>
    <li>💼 I focus on full-stack development.</li>
    <li>🎯 I strive for clean code and agile workflows.</li>
  </ul>

  <h2>🛠️ Tech Stack</h2>
  <ul>
    <li><strong>Programming Languages:</strong> JavaScript, TypeScript, Python</li>
    <li><strong>Frontend:</strong> Angular</li>
    <li><strong>Backend:</strong> Django, Django REST Framework (DRF)</li>
    <li><strong>Tools & Workflows:</strong> Git, Docker</li>
  </ul>

  <h2>🚀 Projects</h2>
  <ul>
    <li><a href="https://github.com/lehmand/DABubble">DABubble</a> – Discord/Slack type Chat-App made with Angular and Firebase</li>
    <li><a href="https://github.com/lehmand/videoflix_backend">Videoflix</a> – Backend for my Netflix clone made with Python/Django</li>
  </ul>

  <h2>📫 Get in Touch</h2>
  <ul>
    <li>✉️ Email: <a href="mailto:contact@daniel-lehmann.dev">contact@daniel-lehmann.dev</a></li>
    <li>💼 LinkedIn: <a href="https://www.linkedin.com/in/daniel-lehmann-856358324/">LinkedIn Profile</a></li>
    <li>🌐 Website: <a href="https://daniel-lehmann.dev">daniel-lehmann.dev</a></li>
  </ul>
</body>
